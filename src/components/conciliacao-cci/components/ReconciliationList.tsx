...
const ReconciliationList: React.FC<ReconciliationListProps> = ({
  ...
  const summary = useMemo(() => {
    return transactions.reduce((acc, t) => {
      if (t.type === 'CREDIT') acc.credits += Math.abs(t.amount);
      else if (t.type === 'DEBIT') acc.debits += Math.abs(t.amount);
      acc.total += t.amount;
      return acc;
    }, { credits: 0, debits: 0, total: 0 });
  }, [transactions]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 relative pb-32 max-w-[1700px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-4 duration-700">
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex items-center gap-5 group hover:shadow-xl hover:shadow-emerald-500/5 transition-all">
        ...
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex items-center gap-5 group hover:shadow-xl hover:shadow-rose-500/5 transition-all">
          <div className="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-500 shadow-inner group-hover:scale-110 transition-transform">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-rose-600 tabular-nums">
              {summary.debits.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
            </p>
          </div>
        </div>

        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex items-center gap-5 group hover:shadow-xl hover:shadow-slate-500/5 transition-all">
          <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 shadow-inner group-hover:scale-110 transition-transform">
            <Wallet className="w-7 h-7" />
          </div>
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Saldo Final</p>
            <p className={`text-2xl font-black tabular-nums ${summary.total >= 0 ? 'text-slate-900' : 'text-rose-600'}`}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between bg-white/95 p-6 rounded-[32px] border border-slate-200 shadow-sm sticky top-0 z-[60] gap-4">
        <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl">
          <button onClick={() => setFilter(TransactionStatus.PENDING)} className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${filter === TransactionStatus.PENDING ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onComplete} disabled={pendingCount > 0} className={`flex items-center gap-2 px-8 py-3 rounded-2xl text-sm font-black transition-all shadow-xl ${pendingCount > 0 ? 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200' : 'bg-red-600 text-white hover:bg-red-700 active:scale-95 shadow-red-200'}`}>
          ...
          <table className="w-full text-left border-collapse min-w-[1400px] table-fixed">
            <thead className="sticky top-0 z-40 bg-white">
              <tr className="bg-slate-50/50 border-b border-slate-200">
                ...
                <th className="px-6 py-5 text-[11px] font-black text-red-600 uppercase tracking-[0.2em] bg-red-50/30 w-52">IA Sugestão</th>
                ...
                <tr key={t.id} className={`group transition-all duration-300 ${t.status === TransactionStatus.REVIEWED ? 'bg-emerald-50/30' : 'hover:bg-slate-50/50'} ${editingId === t.id ? 'z-[90] relative bg-white shadow-2xl' : 'z-0 relative'}`}>
                  <td className="px-6 py-6 text-center">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <input type="date" value={t.date} onChange={(e) => onUpdate(t.id, { date: e.target.value })} className="bg-transparent text-xs font-black text-slate-800 outline-none p-1 transition-all focus:text-red-600" />
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-6 overflow-hidden">
                    <input type="text" value={t.entity || ''} onChange={(e) => onUpdate(t.id, { entity: e.target.value })} placeholder="Identificar..." className="w-full bg-transparent text-xs text-slate-700 outline-none font-bold py-1 border-b border-transparent group-hover:border-slate-200 focus:border-red-500" />
                  </td>
                  ...
                  <td className="px-6 py-6 bg-red-50/10">
                    <div className="text-[11px] font-black text-red-700 truncate uppercase">{t.suggestedAccount || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-6 relative">
                    <div className={`w-full text-left text-[11px] font-black px-4 py-3 rounded-2xl border flex items-center gap-2 ${t.status === TransactionStatus.REVIEWED ? 'bg-slate-50 border-slate-200 text-slate-500' : 'bg-white border-red-200 text-red-900'}`}>
                      <Database className="w-4 h-4 shrink-0 text-red-400" />
                      <input
                        type="text"
                        ...
                        <div className="max-h-[320px] overflow-y-auto p-2 space-y-1 bg-white custom-account-scroll">
                          {(Object.entries(filteredAccountGroups) as [string, string[]][]).map(([group, accounts]) => (
                            <div key={group}>
                              ...
                              {accounts.map((acc) => (
                                <button key={acc} onClick={() => { onUpdate(t.id, { accountingAccount: acc }); setEditingId(null); }} className={`w-full text-left px-4 py-3 text-[13px] rounded-2xl font-bold transition-all ${t.accountingAccount === acc ? 'bg-red-600 text-white' : 'hover:bg-slate-50 text-slate-600'}`}>
                                  {acc}
                                </button>
                              ))}
                            </div>
                  ...
                  <td className="px-6 py-6 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {t.status === TransactionStatus.PENDING ? (
                        ...
                        <button onClick={() => onUpdate(t.id, { status: TransactionStatus.PENDING })} className="text-[10px] font-black text-slate-400 hover:text-red-600 uppercase tracking-widest transition-colors">Reabrir</button>
                      )}
