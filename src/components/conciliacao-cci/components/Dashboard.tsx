...
const Dashboard: React.FC<DashboardProps> = ({ transactions, onNavigateToReconciliation, onNavigateToTutorials }) => {
  ...
  const tips = useMemo(() => [
    { title: 'Limpeza Automática', desc: 'A IA remove códigos como "DOC 123", "TAR" e "PGTO" para que seu histórico fique limpo.', color: 'bg-red-600' },
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-[1600px] mx-auto pb-12">
      ...
      {/* Top Banner - Minimalist White */}
      <div className="relative overflow-hidden bg-white rounded-[48px] p-8 md:p-12 text-slate-900 border border-slate-200 shadow-xl shadow-slate-100/50">
        <div className="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-slate-50 to-transparent"></div>

        <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 border border-slate-200 rounded-2xl text-slate-600 text-[10px] font-black uppercase tracking-widest">
              <Zap className="w-4 h-4 text-amber-500" />
              Intelligence Dashboard
            </div>
            <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-tight text-slate-900">
              Seu escritório está <span className="text-emerald-500">{intelligenceScore}%</span> mais inteligente hoje.
            </h1>
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <button
                ...
                className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-black transition-all flex items-center gap-2 group shadow-xl shadow-slate-200"
              >
                Começar Conciliação
          ...
          <div className="shrink-0 relative">
            <div className="w-48 h-48 md:w-64 md:h-64 rounded-full border-[12px] border-slate-50 flex items-center justify-center relative group">
              <div className="absolute inset-0 rounded-full border-[12px] border-emerald-500 border-t-transparent animate-[spin_3s_linear_infinite] opacity-20"></div>
              <div className="text-center">
                <Trophy className="w-12 h-12 text-amber-500 mx-auto mb-2" />
                <p className="text-4xl font-black text-slate-900">{intelligenceScore}%</p>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Auto-Precisão</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[48px] border border-slate-200 p-10 shadow-sm relative">
          <h3 className="text-2xl font-black text-slate-900 mb-8 flex items-center gap-3">
            <MousePointer2 className="text-slate-400 w-7 h-7" />
            Trilha de Conhecimento {dashboardSearch && <span className="text-sm font-bold text-slate-400">(Filtrado)</span>}
          </h3>
          ...
          <div className="flex gap-4 mb-10 overflow-x-auto pb-4 scrollbar-hide">
            {(dashboardSearch ? filteredSteps : steps).map((step) => (
              <button
                ...
                className={`flex-1 min-w-[140px] p-5 rounded-3xl border-2 transition-all ${activeStep === step.id
                  ? 'border-slate-900 bg-slate-50 shadow-lg shadow-slate-100'
                  : 'border-slate-100 hover:border-slate-200'
                  }`}
              >
                <div className={`w-12 h-12 rounded-2xl ${activeStep === step.id ? 'bg-slate-900' : 'bg-slate-100'} flex items-center justify-center mb-4 shadow-sm`}>
                  <step.icon className={`${activeStep === step.id ? 'text-white' : 'text-slate-400'} w-6 h-6`} />
                </div>
                <p className={`text-xs font-black uppercase tracking-widest ${activeStep === step.id ? 'text-slate-900' : 'text-slate-400'}`}>
                  {step.title}
                </p>
              </button>
            ))}
        ...
        {/* Simulador Interativo - Minimalist White */}
        <div className="bg-white rounded-[48px] p-10 text-slate-900 border border-slate-200 shadow-xl shadow-slate-100/50 relative overflow-hidden flex flex-col">
          <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 -mr-20 -mt-20 rounded-full blur-3xl"></div>
          ...
          <div className="relative z-10 flex-1">
            ...
            <div className="space-y-4">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm font-bold placeholder:text-slate-300 focus:bg-white focus:border-slate-300 outline-none transition-all shadow-inner"
              />

              <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 min-h-[100px] flex flex-col justify-center">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Resultado da Predição</p>
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center space-y-8">
          ...
          <div className="relative max-w-2xl mx-auto">
            ...
            <input
              ...
              className="w-full pl-16 pr-8 py-6 bg-slate-50 border border-slate-100 rounded-[32px] text-lg font-bold outline-none focus:bg-white focus:border-red-300 shadow-inner transition-all"
            />
            {dashboardSearch && (
              <button
