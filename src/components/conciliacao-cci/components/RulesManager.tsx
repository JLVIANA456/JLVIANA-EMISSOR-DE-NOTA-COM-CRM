    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-[1400px] mx-auto relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-4 bg-red-600 text-white rounded-2xl font-black text-sm shadow-xl shadow-red-200 hover:bg-red-700 hover:shadow-red-300 transition-all active:scale-95"
        >
      ...
      <div className="bg-white p-4 rounded-[32px] border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-transparent rounded-2xl text-sm outline-none focus:bg-white focus:border-red-200 transition-all"
          />
        </div>
        ...
        {filteredRules.length > 0 ? (
          filteredRules.map((rule) => (
            <div key={rule.id} className="group bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 bg-slate-50 -mr-8 -mt-8 rounded-full opacity-50 group-hover:scale-110 transition-transform"></div>

              <div className="flex items-start justify-between mb-6 relative z-10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center">
                    <Zap className="w-5 h-5 text-slate-400" fill="currentColor" />
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Padrão de Texto</p>
              ...
              <div className="space-y-4 relative z-10">
                <div className="flex items-center gap-4 py-3 px-4 bg-slate-50 rounded-2xl">
                  ...
                  <div className="flex-1 text-right">
                    ...
                    <span className="inline-block px-2 py-0.5 bg-slate-100 text-slate-600 text-[9px] font-black rounded-md">
                      {rule.scope}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-black text-slate-800 truncate">{rule.account}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:bg-white focus:border-red-300 transition-all shadow-inner"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-6 flex gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black text-sm shadow-xl shadow-red-200 hover:bg-red-700 hover:shadow-red-300 transition-all active:scale-95 flex items-center justify-center gap-2"
                >
      ...
      {/* Summary Footer - Minimalist White */}
      <div className="bg-white p-8 rounded-[40px] text-slate-900 border border-slate-200 shadow-xl shadow-slate-100/50 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
        ...
        <div className="flex items-center gap-4 relative z-10">
          <div className="w-14 h-14 bg-slate-100 rounded-2xl flex items-center justify-center border border-slate-200">
            <Zap className="w-7 h-7 text-amber-500" fill="currentColor" />
          </div>
          <div>
            <h4 className="font-bold text-lg text-slate-900">Automação Ativa</h4>
            <p className="text-slate-500 text-sm opacity-80">Suas regras estão processando {rules.length} padrões conhecidos.</p>
          </div>
        </div>
        <div className="flex items-center gap-8 relative z-10">
          <div className="text-center">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Precisão Estimada</p>
            <p className="text-2xl font-black text-slate-900">98.4%</p>
          </div>
          <div className="w-px h-10 bg-slate-200"></div>
          <div className="text-center">
            <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-1">Horas Poupadas</p>
            <p className="text-2xl font-black text-slate-900">+142h</p>
          </div>
        </div>
      </div>
    </div>
