...
import { analyzeTransactions } from './aiService';
...
const ConciliadorApp: React.FC = () => {
  ...
  const isEmbedded = true; // Forçado para integração nativa

  // 📡 Comunicação com Sistema Pai (Iframe)
  const notifyParent = useCallback((type: string, payload: any) => {
    if (isEmbedded && window.parent !== window) {
      window.parent.postMessage({
        source: 'conciliador-bancario',
        type,
        payload
      }, '*');
      console.log(`[Embedded] Notificando sistema pai: ${type}`, payload);
    }
  }, [isEmbedded]);

  // Efeito para escutar comandos do sistema pai
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // Exemplo: O sistema pai pode enviar um comando para mudar a aba ou enviar dados
      if (event.data?.source === 'sistema-erp') {
        console.log('[Embedded] Mensagem recebida do sistema pai:', event.data);
  ...
  const handleExportTxt = useCallback(() => {
    ...
    // Notificar sistema pai sobre exportação
    notifyParent('EXPORT_STARTED', { count: reviewed.length });
    ...
    notifyParent('EXPORT_FINISHED', { fileName: link.download });
  }, [transactions, selectedBank, notifyParent]);
  ...
  const handleCompleteBatch = useCallback(() => {
    ...
    // Notificar sistema pai sobre lote finalizado
    notifyParent('BATCH_COMPLETED', newBatch);

    setTransactions([]);
    setCurrentFileName(null);
    setActiveTab('dashboard');
  }, [handleExportTxt, currentFileName, selectedBank, transactions, notifyParent]);
  ...
  return (
    <div className="flex h-screen bg-[#fafafa] overflow-hidden font-sans">
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 relative`}>
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-red-500/5 blur-[120px] -z-10 rounded-full" />
        <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-red-600/5 blur-[100px] -z-10 rounded-full" />
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 relative scrollbar-hide">
          <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-700">
            ...
            {activeTab === 'history' && (
              <HistoryList
                history={history}
                onNewBatch={() => setActiveTab('dashboard')}
              />
            )}
            {activeTab === 'tutorials' && (
              <Tutorials />
