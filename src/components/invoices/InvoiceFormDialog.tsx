...
import { ImagePlus, X, Clipboard, Loader2, Search } from "lucide-react";
...
export function InvoiceFormDialog({ open, onOpenChange, onCreated, editingRequest }: Props) {
  ...
  const consultCNPJ = async () => {
    if (!formRef.current) return;
    const cnpjInput = formRef.current.elements.namedItem('client_document') as HTMLInputElement;
    ...
    try {
      ...
      // Update fields
      updateFormField('client_name', data.razao_social || data.nome_fantasia || "");
      updateFormField('client_address', data.logradouro || "");
      updateFormField('client_address_number', data.numero || "");
      updateFormField('client_address_complement', data.complemento || "");
      updateFormField('client_neighborhood', data.bairro || "");
      updateFormField('client_city', data.municipio || "");
      updateFormField('client_state', data.uf || "");
      updateFormField('client_zip_code', data.cep?.replace(/\D/g, "") || "");
      updateFormField('client_email', data.email || "");
      updateFormField('client_phone', data.ddd_telefone_1 || data.telefone || "");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      cClassTrib: get('cClassTrib') || null,
      cIndOp: get('cIndOp') || null,
      indFinal: get('indFinal') || '0',
      indDest: get('indDest') || '0',
    };
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Input 
                    ...
                    defaultValue={editingRequest?.client_document || ''} 
                    key={`doc-${editingRequest?.id || 'new'}`} 
                  />
                  <Button 
                    type="button" 
                    variant="secondary" 
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {/* CAMPOS REFORMA TRIBUTÁRIA / NOVO LAYOUT */}
              <div className="col-span-2 mt-4 pt-4 border-t">
                  <h4 className="text-xs font-semibold text-muted-foreground mb-3">Reforma Tributária (Opcional)</h4>
                  <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <Label>cClassTrib (Código de Tributação)</Label>
                        <Input name="cClassTrib" placeholder="Ex: 1001" defaultValue={editingRequest?.cClassTrib || ''} />
                      </div>
                      <div className="space-y-1.5">
                        <Label>cIndOp (Indicador de Operação)</Label>
                        <Input name="cIndOp" placeholder="Ex: 000001" defaultValue={editingRequest?.cIndOp || ''} />
                      </div>
                      <div className="space-y-1.5">
                        <Label>indFinal (Consumidor Final?)</Label>
                        <Select name="indFinal" defaultValue={editingRequest?.indFinal || '0'}>
                            <SelectTrigger><SelectValue /></SelectTrigger>
                            <SelectContent>
                      ...
                      <div className="space-y-1.5">
                        <Label>indDest (Destino da Operação)</Label>
                        <Select name="indDest" defaultValue={editingRequest?.indDest || '0'}>
                            <SelectTrigger><SelectValue /></SelectTrigger>
                            <SelectContent>
                                <SelectItem value="0">Normal</SelectItem>
                                <SelectItem value="1">Uso/Consumo</SelectItem>
                            </SelectContent>
                        </Select>
                      </div>
                  </div>
