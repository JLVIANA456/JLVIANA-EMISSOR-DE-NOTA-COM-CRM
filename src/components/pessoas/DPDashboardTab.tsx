...
export function DPDashboardTab({ people, contracts, absences, sheets, totalMonthlySalary }: Props) {
  ...
  const cards = [
    { title: "Clientes Ativos", value: active, sub: `${paused} suspensos · ${inactive} inativos`, icon: Users, color: "text-primary" },
    ...
    { title: "Solicitações Pendentes", value: upcomingAbsences.length, sub: "Próximos 14 dias", icon: Calendar, color: "text-amber-500" },
    { title: "Tarefas em Aberto", value: pendingApprovalAbsences + pendingSheets.length, sub: `${pendingApprovalAbsences} chamados · ${pendingSheets.length} faturamentos`, icon: AlertTriangle, color: (pendingApprovalAbsences + pendingSheets.length) > 0 ? "text-amber-500" : "text-muted-foreground" },
    { title: "Receita Recorrente (MRR)", value: fmt(totalMonthlySalary), sub: `${active} clientes ativos`, icon: Wallet, color: "text-primary", isText: true },
  ];
