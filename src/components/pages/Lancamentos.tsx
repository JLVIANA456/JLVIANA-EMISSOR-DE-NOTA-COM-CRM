...
import {
    ...
    TrendingUp, TrendingDown, Landmark, Scale, Info, ChevronRight, FilterX, Loader2, FileUp
} from "lucide-react";
...
const Lancamentos = () => {
    ...
    const fetchRules = async () => {
        if (!selectedClient) return;
        // Rules can be stored in a JSON column in configuracoes_sistema or a new table
        // For now, let's look for a 'finance_rules' table or fallback to local defaults
        const { data } = await (supabase as any)
            .from('finance_rules')
            .select('*')
            .eq('empresa_id', selectedClient.id);

        if (data) setAutoRules(data);
        else {
            // Default rules if table doesn't exist
            setAutoRules([
                { keyword: 'Posto', category_name: 'Combustível / Pedágio' },
                { keyword: 'Uber', category_name: 'Transporte' },
                { keyword: '99', category_name: 'Transporte' },
                { keyword: 'Ifood', category_name: 'Alimentação' },
                { keyword: 'Restaurante', category_name: 'Alimentação' },
                { keyword: 'Amazon', category_name: 'Infraestrutura / TI' }
            ]);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (form.id) {
                const { error } = await (supabase as any).from('finance_transactions').update(payload).eq('id', form.id);
                if (error) throw error;
                ...
                if (form.is_recurring) {
                    ...
                    for (let i = 0; i < 12; i++) {
                        ...
                        entries.push({
                            ...
                            descricao: `${form.descricao} (${i + 1}/12)`
                        });
                    }
                    const { error } = await (supabase as any).from('finance_transactions').insert(entries);
                    if (error) throw error;
                    toast.success("12 mensalidades geradas com sucesso!");
                } else {
    ...
    const applyRules = (description: string) => {
        ...
        for (const rule of autoRules) {
            if (lowerDesc.includes(rule.keyword.toLowerCase())) {
                const cat = categories.find(c => c.nome.toLowerCase().includes(rule.category_name.toLowerCase()));
        ...
        while ((match = transRegex.exec(ofxString)) !== null) {
            ...
            if (rawDate) {
                ...
                transactions.push({
                    ...
                    status: 'pendente',
                    categoria_id: applyRules(description)
                });
            }
        }
        return transactions;
    ...
    const handleOFXImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            const toastId = toast.loading(`Importando ${parsed.length} lançamentos do OFX...`);
            ...
            const { error } = await (supabase as any).from('finance_transactions').insert(finalData);

            if (error) {
                toast.error("Erro na importação: " + error.message, { id: toastId });
            } else {
                toast.success(`${parsed.length} lançamentos importados e categorizados por regras!`, { id: toastId });
                fetchTransactions();
            }
        };
    ...
    const handleExport = () => {
        ...
        const exportData = filteredTransactions.map(tr => ({
            Data: new Date(tr.data).toLocaleDateString('pt-BR'),
            Descrição: tr.descricao,
            Valor: tr.valor,
            Tipo: tr.tipo === 'entrada' ? 'Receita' : 'Despesa',
            Categoria: tr.finance_categories?.nome || "Outros",
            Conta: tr.finance_bank_accounts?.banco || "-",
            Status: tr.status === 'efetivado' ? 'Efetivado' : 'Pendente'
        }));
        ...
        saveAs(data, `lancamentos_${selectedClient?.nome_fantasia || 'export'}.xlsx`);
    ...
    const handleBulkImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (evt) => {
            ...
            const toastId = toast.loading(`Importando ${data.length} registros...`);

            for (const row of data as any[]) {
                try {
                    // Try to find category by name
                    const categoryName = row.Categoria || row.categoria || "";
                    const category = categories.find(c => c.nome.toLowerCase() === categoryName.toLowerCase());

                    // Try to find bank account by name
                    const accountName = row.Conta || row.conta || "";
                    const bankAccount = bankAccounts.find(ba => ba.banco.toLowerCase().includes(accountName.toLowerCase()));

                    const payload = {
                        data: row.Data || row.data || new Date().toISOString().split('T')[0],
                        descricao: row.Descrição || row.descricao || row.Descricao || "Importação em Lote",
                        valor: Math.abs(Number(row.Valor || row.valor || 0)),
                        tipo: (row.Tipo || row.tipo || '').toString().toLowerCase().includes('receita') || (row.Tipo || row.tipo || '').toString().toLowerCase().includes('entrada') ? 'entrada' : 'saida',
                        categoria_id: category?.id || null,
                        bank_account_id: bankAccount?.id || null,
                        status: (row.Status || row.status || '').toString().toLowerCase().includes('efetivado') ? 'efetivado' : 'pendente',
                        tenant_id: selectedClient.tenant_id,
                        empresa_id: selectedClient.id,
                    };
            ...
            if (errorCount > 0) {
                toast.warning(`Importação concluída: ${importedCount} sucesso, ${errorCount} erros.`);
            } else {
                toast.success(`Sucesso! ${importedCount} lançamentos importados.`);
            }
            fetchTransactions();
            setLoading(false);
        };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 pb-16">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 p-8">
                                ...
                                <div className="space-y-2 md:col-span-2">
                                    ...
                                    <div className="flex items-center space-x-2 pt-4">
                                        ...
                                        />
                                        <Label htmlFor="is_recurring" className="text-xs font-medium text-muted-foreground">
                                            Lançamento Mensal (Gerar previsões para 12 meses)
                                        </Label>
                                    </div>
                                </div>
                            </div>
            ...
            <Card className="bg-card/50 backdrop-blur-md border-border/60 shadow-xl rounded-3xl overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between py-8 px-8 border-b border-border/40">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <Upload className="h-3.5 w-3.5 mr-2 opacity-50" /> Planilha
                        </Button>
                        <Button
                            variant="outline"
                            size="sm"
                        ...
                        >
                            <Landmark className="h-3.5 w-3.5 mr-2 opacity-50" /> OFX
                        </Button>
                        <input
                            type="file"
                            ref={ofxInputRef}
            ...
            <input ref={fileInputRef} type="file" className="hidden" accept="image/*" onChange={handleAILoad} />
        </div>
    );
};
