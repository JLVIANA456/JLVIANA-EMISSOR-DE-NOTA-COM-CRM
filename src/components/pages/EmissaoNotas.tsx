...
const NODE_BRIDGE_URL = "http://127.0.0.1:3000/nfse";
...
const EmissaoNotas = () => {
  ...
  const checkServer = async () => {
    try {
      console.log("Verificando servidor em:", NODE_BRIDGE_URL + "/health");
      const res = await fetch(NODE_BRIDGE_URL + "/health", {
        method: "GET",
        mode: 'cors',
        headers: {
      ...
      setIsServerUp(res.ok && data.status === 'ok');
    } catch (err) {
      console.error("Falha ao detectar servidor Node:", err);
      setIsServerUp(false);
    }
  };
  ...
  const emitirNota = async (request: InvoiceRequest) => {
    console.log("Iniciando emitirNota para request:", request.id);

    if (!isServerUp) {
      toast.error("Servidor de Notas Offline. Verifique se o Node está rodando na porta 3000.");
      checkServer();
      return;
    }

    if (!config.cnpj || !config.im || !config.certificate) {
      toast.warning("Configure o CNPJ, CCM e Certificado antes de emitir.");
      setShowSettings(true);
      return;
    }

    setActionLoading(request.id);
    toast.info("Conectando com a Prefeitura (Lote Assíncrono)...");
    ...
    try {
      const payload = {
        ...
        config: config,
        data: {
          numeroRps: request.id.slice(0, 12).replace(/\D/g, "") || "1",
          serieRps: "1",
          tipoRps: "1",
          dataEmissao: new Date().toISOString().split('T')[0],
          statusRps: "1",
          tributacaoRps: config.naturezaPadrao || "1",
          cnpjPrestador: config.cnpj.replace(/\D/g, ""),
          imPrestador: config.im.replace(/\D/g, ""),
          valorServicos: Number(request.gross_value),
          codigoServico: request.service_code_municipal?.replace(/\D/g, "") || "02881",
          aliquotaServicos: (request.iss_aliq || 2) / 100,
          issRetido: request.iss_retained,
          valorDeducoes: Number(request.deductions_value || 0),
          cpfCnpjTomador: request.client_document.replace(/\D/g, ""),
          razaoSocialTomador: request.client_name,
          logradouro: request.client_address || "Rua Desconhecida",
          numeroEndereco: request.client_address_number || "SN",
          complementoEndereco: request.client_address_complement || "",
          bairro: request.client_neighborhood || "Centro",
          ...
          ibsCbs: {
            finNFSe: "0",
            indFinal: request.indFinal || "0",
            cIndOp: request.cIndOp || "1001",
            indDest: request.indDest || "0",
            cClassTrib: request.cClassTrib || "1001",
            cClassTribReg: request.cClassTrib || "1001",
            tpOper: "1"
          }
        },
      };

      const response = await fetch(NODE_BRIDGE_URL + "/emitir", {
        method: "POST",
      ...
      console.log("RETORNO PMSP SERVICE:", result);
      ...
      if (result.success) {
        if (result.status === 'emitida') {
          ...
          await supabase
            ...
            .update({
              ...
              nfe_number: result.numeroNfe || null,
              verification_code: result.codigoVerificacao || null,
              rps_number: result.rpsNumber || null,
              log: result.message || "Emitida via fluxo assíncrono"
            } as any)
            .eq("id", request.id);

          fetchRequests();
        } else if (result.status === 'processando' || result.status === 'lote_enviado') {
          toast.info(`Lote enviado! Status: ${result.message || 'Aguardando processamento'}`);

          await supabase
            .from("invoice_requests" as any)
            .update({
              status: "emissao_andamento",
              log: result.message || "Lote em processamento na prefeitura"
            } as any)
            .eq("id", request.id);
        ...
        toast.error("Erro na prefeitura: " + errorMsg);
      ...
      toast.error("Erro ao conectar com o serviço de notas (Porta 3000).");
    } finally {
  ...
  const deleteRequest = async (id: string) => {
    try {
      ...
      toast.success("Nota removida da fila.");
      fetchRequests();
    } catch (error) {
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="md:col-span-3 space-y-2">
                    <Label className="text-xs">Certificado Digital (Arquivo .pfx ou Caminho Local)</Label>
                    <div className="flex gap-2">
                      <Input
                        value={config.certificate && config.certificate.length > 100 ? "Certificado carregado (Base64)" : config.certificate}
                        onChange={e => {
                          // Se o usuário editar manualmente, assumimos que é um caminho ou ele está colando algo.
                          // Se ele tentar editar o texto "Certificado carregado...", limpamos tudo.
                        ...
                        placeholder="Caminho (C:/...) ou use o botão Procurar"
                ...
                <CardContent className="p-0">
                  <div className="flex items-stretch">
                    ...
                    <div className="p-4 flex-1 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="ghost" size="sm" onClick={() => deleteRequest(request.id)} className="h-8 text-[11px] font-light text-red-500 hover:text-red-700 hover:bg-red-50">
                          Excluir
                        </Button>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {request.nfe_number && (
                    <div className="text-[10px] font-medium text-primary mt-0.5">
                      NFSe: {request.nfe_number} {request.verification_code && `• Cód: ${request.verification_code}`}
                    </div>
                  )}
