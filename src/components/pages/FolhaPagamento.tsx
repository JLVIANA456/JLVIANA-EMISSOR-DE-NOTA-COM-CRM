...
import {
  ...
  Send, UserPlus, Receipt, AlertTriangle, Clock, ChevronLeft, Eye, BarChart3, Link2, Copy
} from "lucide-react";
...
const STATUS_CONFIG: Record<string, { label: string; variant: "default" | "secondary" | "destructive" | "outline"; color: string }> = {
  rascunho: { label: "Em Digestão", variant: "secondary", color: "text-muted-foreground" },
  pronta: { label: "Aguardando Aprovação", variant: "outline", color: "text-yellow-600" },
  aprovada: { label: "Faturado", variant: "default", color: "text-primary" },
  paga: { label: "Recibo Emitido", variant: "default", color: "text-primary" },
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-light tracking-tight">Faturamento & Recibos BPO</h1>
            <p className="text-sm text-muted-foreground">Gestão de faturamento mensal, emissão de recibos e conferência de Notas Fiscais</p>
          </div>
        </div>
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>Novo Lançamento de Faturamento</DialogTitle></DialogHeader>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-light">Faturamento {MONTHS_FULL[activeSheet.month - 1]} / {activeSheet.year}</h2>
              <div className="flex items-center gap-2 mt-1">
                ...
                {canEmit && (
                  <Badge variant="outline" className="text-[10px]">
                    {emittedCount}/{sheetItems.length} recibos gerados
                  </Badge>
                )}
              </div>
            </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {isEditable && availablePeopleForSheet.length > 0 && (
              <Dialog open={addPjOpen} onOpenChange={setAddPjOpen}>
                <DialogTrigger asChild>
                  <Button size="sm" variant="outline"><UserPlus className="h-4 w-4 mr-1" /> Adicionar Cliente</Button>
                </DialogTrigger>
            ...
            {activeSheet.status === "aprovada" && (
              <>
                <Button size="sm" onClick={() => handleStatusChange(activeSheet.id, "pagar")}>
                  <Check className="h-4 w-4 mr-1" /> Finalizar & Gerar Recibos
                </Button>
                <Button size="sm" variant="outline" onClick={handleEmitAll}>
                  <Send className="h-4 w-4 mr-1" /> Enviar Todos Recibos
                </Button>
              </>
            )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="pb-1"><CardTitle className="text-xs font-light text-muted-foreground">Valor Bruto</CardTitle></CardHeader>
            <CardContent><div className="text-lg font-light font-mono">{fmt(sheetItems.reduce((s, i) => s + Number(i.base_value), 0))}</div></CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-1"><CardTitle className="text-xs font-light text-muted-foreground">Ajustes / Glosas</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader className="pb-1"><CardTitle className="text-xs font-light text-muted-foreground">Adicionais</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader className="pb-1"><CardTitle className="text-xs font-light text-muted-foreground">Total Faturamento</CardTitle></CardHeader>
