...
const PortalPjAuth = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!profile) {
      ...
      if (person) {
        ...
        toast.error("Seu e-mail não está vinculado a nenhum cliente ou colaborador ativo.");
        return;
      }
    }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Check if email exists as active client/person
    const { data: person } = await supabase
      .from("people")
      .select("id, name")
    ...
    if (!person) {
      setLoading(false);
      toast.error("Este e-mail não está cadastrado como cliente ou colaborador ativo. Entre em contato com o suporte.");
      return;
    }
    ...
    if (data.user && data.session) {
      ...
      toast.success("Conta criada! Bem-vindo ao Portal do Cliente!");
      navigate("/portal-pj/dashboard");
    } else {
      ...
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            Acesse seus recibos, envie documentos e acompanhe seu faturamento
          </p>
        </div>
            ...
            <CardContent>
              ...
              <TabsContent value="signup" className="mt-0">
                <CardDescription className="mb-4 text-xs">
                  Use o mesmo e-mail cadastrado na plataforma. Apenas clientes e colaboradores ativos podem criar conta.
                </CardDescription>
