...
export function ContractSignature() {
  ...
  const { data: contracts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("contracts")
        .select("id, title, status, clicksign_status, clicksign_document_key, generated_content, final_pdf_url, created_at, style_config")
  ...
  const callClicksign = async (action: string, extra: Record<string, any> = {}) => {
    // Check if we are in local development and use PHP proxy if so
    const isLocal = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1";
    
    if (isLocal) {
       // Use local PHP proxy
       const resp = await fetch("http://localhost:8001/clicksign_proxy.php", {
          method: "POST",
          headers: {
       ...
       if (!resp.ok) {
         ...
         throw new Error(err.error || `Erro PHP Proxy ${resp.status}`);
       }
       return resp.json();
    }

    // Default to Supabase Edge Function for production
  ...
  const getFontInfo = (family: string) => {
    switch (family) {
  ...
  const generatePdfBase64 = (content: string, styles?: any): string => {
    ...
    const s = {
      ...
      marginTop: styles?.marginTop || 25,
      marginBottom: styles?.marginBottom || 25,
      marginLeft: styles?.marginLeft || 30,
      marginRight: styles?.marginRight || 20,
    };

    const fontName = getFontInfo(s.fontFamily);
    const pageWidth = 210;
    const pageHeight = 297;
    const effectiveWidth = pageWidth - s.marginLeft - s.marginRight;
    const lineHeightMm = s.fontSize * 0.3527 * s.lineHeight;
    
    let cursorY = s.marginTop;
    ...
    for (const rawLine of lines) {
      ...
      const isHeader = rawLine.startsWith("#") || rawLine.startsWith("**") || /^CL[ÁA]USULA/i.test(line);
      ...
      if (!line) {
        ...
        if (cursorY > pageHeight - s.marginBottom) { pdf.addPage(); cursorY = s.marginTop; }
        continue;
      }

      const splitLines: string[] = pdf.splitTextToSize(line, effectiveWidth);
      
      for (const sl of splitLines) {
        if (cursorY + lineHeightMm > pageHeight - s.marginBottom) { pdf.addPage(); cursorY = s.marginTop; }
        
        // Alignment
        let xPos = s.marginLeft;
        if (s.textAlign === 'center' || isHeader) {
          xPos = pageWidth / 2 - (pdf.getStringUnitWidth(sl) * s.fontSize / 2.83465) / 2;
        } else if (s.textAlign === 'right') {
           xPos = pageWidth - s.marginRight - (pdf.getStringUnitWidth(sl) * s.fontSize / 2.83465);
        }

        pdf.text(sl, xPos, cursorY, {
             align: (s.textAlign === 'justify' && !isHeader) ? 'justify' : 'left',
  ...
  const handleUploadToClicksign = async (contractId: string) => {
    ...
    try {
      // Find contract inside contracts array
      // Note: contracts is typed as 'any' in the original code map, but here we need to access it
      const contract = (contracts as any[]).find((c: any) => c.id === contractId);
      if (!contract?.generated_content) throw new Error("Conteúdo do contrato vazio");

      // Generate PDF locally to send as base64 (needed for PHP proxy)
      const pdfBase64 = generatePdfBase64(contract.generated_content, contract.style_config);
      
      await callClicksign("upload_document", { 
         contractId, 
         title: contract.title,
         pdfBase64 // Pass base64 PDF to backend
      });
  ...
  const generatePdfPreview = (content: string, styles?: any) => {
    const pdf = new jsPDF({ format: "a4", unit: "mm" });
                      ...
                      {!hasClicksignKey ? (
                        <>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => generatePdfPreview(contract.generated_content, contract.style_config)}
                          >
