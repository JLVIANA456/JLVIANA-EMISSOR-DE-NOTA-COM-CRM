...
import { Loader2, Sparkles, FileText, Plus, Eye, Brain, FileCheck, Search, AlertTriangle, FileDown, Download } from "lucide-react";
...
export function ContractGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    if (form.category === 'contabilidade') {
      ...
      content = content.replace("[CONTRATANTE_NOME]", form.company_razao_social || "[RAZÃO SOCIAL]");
      content = content.replace("[CONTRATANTE_CNPJ]", form.company_cnpj || "[CNPJ]");
      content = content.replace("[CONTRATANTE_ENDERECO]", form.company_address || "[ENDEREÇO]");
      content = content.replace("[CONTRATANTE_EMAIL]", form.company_email || "[EMAIL]");
      content = content.replace("[HONORARIOS_MENSAIS]", form.contract_value || "[VALOR]");
      content = content.replace("[HONORARIOS_MENSAIS_EXTENSO]", form.contract_value_extenso || "[VALOR POR EXTENSO]");
      content = content.replace("[DATA_VENCIMENTO]", form.due_day || "10");
      content = content.replace("[DATA_INICIO]", form.start_date || "[DATA INÍCIO]");
      content = content.replace("[PARAM_FUNCIONARIOS]", form.param_funcionarios);
      content = content.replace("[PARAM_SOCIOS]", form.param_socios);
      content = content.replace("[PARAM_CONTAS]", form.param_contas);
      content = content.replace("[PARAM_ESTABELECIMENTOS]", form.param_estabelecimentos);
      content = content.replace("[PARAM_REGIME]", form.param_regime);
      content = content.replace("[PARAM_ATIVIDADE]", form.param_atividade);
      content = content.replace("[DATA_ASSINATURA]", form.signing_date);
  ...
  const exportToWord = async () => {
    if (!generatedContent || !form.company_razao_social) return;
  ...
  const exportToPDF = () => {
    ...
    docUrl.setFont("helvetica", "normal");
    docUrl.setFontSize(11);

    const lines = docUrl.splitTextToSize(generatedContent, 170);
    const pageHeight = docUrl.internal.pageSize.height;
    let cursorY = 20;
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground font-light uppercase tracking-widest">Documentos Gerados e Templates</p>
            </div>
          </div>
                  ...
                  {contracts.map((c: any) => (
                    <TableRow key={c.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="rounded-lg h-9" onClick={() => setViewContract(c)}>
        ...
        <Dialog open={!!viewContract} onOpenChange={() => setViewContract(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] rounded-[2rem] border-none shadow-2xl overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-primary to-blue-600" />
            <DialogHeader className="p-8">
              <DialogTitle className="text-2xl font-light tracking-tight">{viewContract?.title}</DialogTitle>
              <DialogDescription className="uppercase tracking-widest text-[10px] text-muted-foreground font-bold italic">Documento Contábil Estruturado</DialogDescription>
            </DialogHeader>
            <ScrollArea className="h-[70vh] px-8 pb-8">
              <div className="prose prose-sm max-w-none whitespace-pre-wrap font-serif text-slate-800 leading-relaxed bg-white p-12 rounded-xl shadow-inner border border-slate-100">
                {viewContract?.generated_content || "Sem conteúdo gerado."}
              </div>
            </ScrollArea>
          </DialogContent>
        </Dialog>
  ...
  return (
    <Card className="border-none shadow-2xl rounded-[3rem] overflow-hidden bg-white">
      <div className="h-3 bg-gradient-to-r from-emerald-500 to-primary" />
      <CardHeader className="p-12 pb-6">
        <div className="flex items-center justify-between">
          <CardTitle className="text-3xl font-light tracking-tight flex items-center gap-3">
            <FileText className="h-8 w-8 text-primary" /> Contrato Contábil Gerado
          </CardTitle>
          <Badge className="bg-emerald-500/10 text-emerald-600 border-none px-4 py-1.5 uppercase font-bold text-[10px] tracking-widest">Estrutura Validada pela IA</Badge>
        </div>
        <CardDescription className="text-sm font-light text-muted-foreground mt-2">Revise o documento preenchido automaticamente pela IA da JL Viana.</CardDescription>
      </CardHeader>
      <CardContent className="p-12 pt-0 space-y-8">
        <div className="relative group">
          <ScrollArea className="h-[65vh] border-2 border-slate-100 rounded-[2rem] p-0 shadow-inner bg-slate-50/30">
            <div className="prose prose-sm max-w-2xl mx-auto whitespace-pre-wrap font-serif text-slate-800 leading-relaxed bg-white p-12 md:p-20 shadow-xl border border-slate-100 my-8">
              {generatedContent}
            </div>
          </ScrollArea>
          <div className="absolute inset-0 pointer-events-none rounded-[2rem] border border-primary/5 group-hover:border-primary/20 transition-colors" />
        </div>

        <div className="flex items-center gap-3 p-6 bg-amber-50 rounded-2xl border border-amber-100/50">
          <AlertTriangle className="h-5 w-5 text-amber-500 shrink-0" />
          <p className="text-xs text-amber-800 font-light leading-relaxed">
            ⚠️ <strong>Importante:</strong> Este documento foi gerado com base no modelo oficial da <strong>JL Viana Consultoria Contábil</strong>.
            Antes da assinatura oficial, realize uma leitura minuciosa de todos os dados variáveis preenchidos.
          </p>
        </div>

        <div className="flex justify-between gap-3">
          <div className="flex gap-2">
            <Button variant="outline" className="font-light h-12 px-6 rounded-xl border-border/50 hover:bg-muted" onClick={exportToWord}>
              <FileDown className="h-4 w-4 mr-2 text-indigo-500" /> Exportar DOCX
            </Button>
            <Button variant="outline" className="font-light h-12 px-6 rounded-xl border-border/50 hover:bg-muted" onClick={exportToPDF}>
              <Download className="h-4 w-4 mr-2 text-rose-500" /> Exportar PDF
            </Button>
          </div>
          <div className="flex gap-2">
            {!viewContract && <Button variant="ghost" className="font-light h-12 px-6 rounded-xl" onClick={() => setStep(3)}>Ajustar Dados</Button>}
            {viewContract ? (
              <Button variant="ghost" className="font-light h-12 px-8 rounded-xl" onClick={() => { setStep(0); setViewContract(null); }}>Voltar para Lista</Button>
            ) : (
              <Button onClick={handleSave} className="rounded-xl h-12 px-10 bg-primary shadow-xl shadow-primary/30 text-white font-bold transition-all hover:scale-105 active:scale-95">
                <FileCheck className="h-5 w-5 mr-2" /> Salvar Contrato na Gestão
              </Button>
            )}
          </div>
        </div>
